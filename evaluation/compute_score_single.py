...
P1_final = P1_exp / 10  # number of splits
P3_final = P3_exp / 10
hdf.close()

P1_final = np.round(P1_final * 100, 3)
P3_final = np.round(P3_final * 100, 3)
if criterion == "precision":
    print(
        f"\tAverage P@1 performance over all splits: {P1_final:.2f}.")
    print(
        f"\tAverage P@3 performance over all splits: {P3_final:.2f}.")
elif criterion == "matching" and top_k == 3:
    print(
        f"\tAverage top-3 matching over all splits: {P3_final:.2f}.")
else:
